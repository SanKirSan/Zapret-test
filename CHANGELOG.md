...
## 0.6.7.1 - 2026-09-11
- Убран лишний экран «ВЫБОР ПРОФИЛЯ» из прямой диагностики blockcheck: выбор сервиса выполняется один раз непосредственно в диагностике.
- Диагностика blockcheck больше не требует предварительного `services.list`; список сервисов строится напрямую из актуального runtime-конфига.
- Версия релиза синхронизирована во всех build/release metadata.

# Changelog

## 0.6.6.10 - 2026-09-11
- Raw blockcheck reports are retained automatically, including interrupted runs.
- AVAILABLE blockcheck candidates are persisted for later inspection even when the scan is interrupted.
- Blockcheck console output filters known BusyBox arithmetic-noise lines for readability.
- The report path and raw-capture directory are printed after interrupted blockcheck runs.

## 0.6.6.9

- Исправлена несовместимость BusyBox `tr`: локальное приведение регистра теперь использует диапазоны `A-Z` → `a-z`, поэтому контрольные домены больше не искажаются.
- В прямой диагностике blockcheck после выбора сервиса показывается TCP-проверка всех валидных доменов, после чего можно выбрать любой домен вручную или оставить автоматический target.
- Подтверждённые `PASS`-стратегии без side effects сохраняются в `/tmp/zapret-test/working-strategies/` вместе с движком, аргументами и исходным сервисом/доменом.
- В меню blockcheck добавлен режим `4) Совместимость стратегий`: сохранённые рабочие стратегии запускаются автоматически на всех или выбранных доменах любого сервиса, без ручного выбора стратегий.

# 0.6.6.8 — 2026-09-11

- Обновление доменных списков стало атомарным и авторитетным для соответствующего сервиса: успешный источник заменяет домены сервиса, а не объединяется с устаревшим bundled-списком.
- Добавлен fallback источника списков через jsDelivr при недоступности raw.githubusercontent.com.
- Вывод обновления списков теперь показывает «без изменений» или время последнего успешного обновления cache.
- `ncat` имеет приоритет над `nc` во всех TCP-проверках, включая расширенную NVIDIA-диагностику.
- После `Ctrl-C`/SIGTERM во время blockcheck проверка `Sxxxx/Cxxxx` не запускается вообще.
- Приветствие «Введите для начала работы...» перенесено из `install.sh` в интерактивный запуск `zapret-test`.
- Исправлены ссылки на `bootstrap.sh` в README.
